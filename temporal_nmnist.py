...
def train_model(config):
   ...
   for e in range(params['Network']['epochs']):
       ...
       if val_acc == best_val: #saves the best validation trial? not total because no more global var.
           ...
           checkpoint_file = os.path.join("/data/meganfu/dump/N-MNIST/best", f"{val_name}.pt")
           torch.save(
       ...
       if e == (params['Network']['epochs'])-1:
           val_name = datetime.now()
           val_dict = net.state_dict()
           checkpoint_file = os.path.join("/data/meganfu/dump/N-MNIST/latest", f"{val_name}.pt")
           torch.save(
...
def last(metrics, results):
   ...
   checkpoint_file = os.path.join("/data/meganfu/dump/N-MNIST/checkpoint.pt")


   for filename in os.listdir(os.path.join("/data/meganfu/dump/N-MNIST", best_last)): # if want to look at latest, need another one to load.
       file_path = os.path.join("/data/meganfu/dump/N-MNIST", best_last, filename)
       checkpoint = torch.load(file_path)
       file_acc = checkpoint["acc"]
       if file_acc >= big_acc:
           big_acc = file_acc
           checkpoint_file = os.path.join("/data/meganfu/dump/N-MNIST", best_last, f"{filename}")
       else:
           continue
   ...
   wandb.init(project = 'Temporal_Ablation', name = (best_last + " testing accuracy - N-MNIST"))
