...
def train_(network, trainloader, opti, epoch, states, network_config, layers_config, err):
   ...
   for batch_idx, (inputs, lengths, labels) in enumerate(trainloader):
...
def test(network, testloader, epoch, states, network_config, layers_config, early_stopping):
   ...
   for batch_idx, (inputs, lengths, labels) in enumerate(testloader):
...
def train_model(config):
   ...
   for e in range(params['Network']['epochs']):
       ...
       test_acc = test(net, val_loader, e, l_states, params['Network'], params['Layers'], early_stopping)
       ...
       train.report({'training_accuracy': train_acc, 'test_accuracy': test_acc,  'trial/best_test': best_val,})  #gets most recent and best global
...
def last(metrics, results):
   ...
   checkpoint_file = os.path.join("/data/meganfu/dump/DVS/checkpoint.pt")


   for filename in os.listdir(os.path.join("/data/meganfu/dump/DVS", best_last)): # if want to look at latest, need another one to load.
       file_path = os.path.join("/data/meganfu/dump/DVS", best_last, filename)
       checkpoint = torch.load(file_path)
       file_acc = checkpoint["acc"]
       if file_acc >= big_acc:
           big_acc = file_acc
           checkpoint_file = os.path.join("/data/meganfu/dump/DVS", best_last, f"{filename}")
       else:
           continue
   ...
   wandb.init(project = 'Temporal_Ablation', name = (best_last + " testing accuracy - DVS"))
...
if params['Network']['dataset'] == "MNIST":
   ...
   data_path = os.path.expanduser(params['Network']['data_path'])
   train_loader, test_loader = loadDVSG.get_dvsg(data_path, params['Network'])
else:
...
tuner = tune.Tuner(
   ...
   run_config=RunConfig(
       callbacks=[WandbLoggerCallback( project="Temporal_Ablation",
                                      excludes = ["params", "train_loader", "test_loader"],
                                      log_config = True, name = "DVS")]
   )
)
