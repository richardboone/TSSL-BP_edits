...
def train_model(config):
   ...
   for e in range(params['Network']['epochs']):
       ...
       if val_acc == best_val: #saves the best validation trial? not total because no more global var.
           ...
           checkpoint_file = os.path.join("/data/meganfu/dump/DVS/best", f"{val_name}.pt")
           torch.save(
       ...
       if e == (params['Network']['epochs'])-1:
           val_name = datetime.now()
           val_dict = net.state_dict()
           checkpoint_file = os.path.join("/data/meganfu/dump/DVS/latest", f"{val_name}.pt")
           torch.save(
...
config = {
       ...
       "n_steps": tune.randint(2,40),
       "desired_count": tune.uniform(0.2,1),
       "undesired_count": tune.uniform(0,0.2),
