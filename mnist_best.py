...
def train_model(config):
   ...
   for e in range(params['Network']['epochs']):
       ...
       test_acc = test(net, test_loader, e, l_states, params['Network'], params['Layers'], early_stopping)
       ...
       train.report({'training_accuracy': train_acc, 'testing_accuracy': test_acc, 'trial/best_test': best_val,})  #gets most recent and best global
...
def last(metrics, results):
   ...
   best_last = ""
   if metrics == "training_acc":
       best_last = "best"
   elif metrics == "trial/best_train":
       best_last = "latest"
