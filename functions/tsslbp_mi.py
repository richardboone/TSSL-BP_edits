...
class TSSLBP_MI(torch.autograd.Function): 
    @staticmethod
    def forward(ctx, inputs, init_membranes, network_config, layer_config):
        shape = inputs.shape
        n_steps = shape[4] 
        ...
        mem = torch.zeros(shape[0], shape[1], shape[2], shape[3]).cuda() + init_membranes
